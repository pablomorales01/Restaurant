...
class UiController extends Controller
{
    ...
    */
    public function actionUserManagementCreate()
    {
        ...
        if (isset($_POST[CrugeUtil::config()->postNameMappings['CrugeStoredUser']]) && isset ($_POST['Usuario'])) {
            $model->attributes = $_POST[CrugeUtil::config()->postNameMappings['CrugeStoredUser']];
            ...
            if ($model->validate()) {
                ...
                if (Yii::app()->user->um->save($model, 'insert') && $usuario->save()) {

                    $this->onNewUser($model, $newPwd);
                    $usuario->USU_ID = $model->iduser; //tabla personal recive el iduser generado por cruge
                    $usuario->save();
        ...
        $this->render("usermanagementcreate", array('model' => $model, 'usuario'=>$usuario));
    }
